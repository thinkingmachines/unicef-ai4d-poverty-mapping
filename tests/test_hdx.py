from relativewealth.hdx import download_hdx, get_hdx_file
from pathlib import Path

def test_download_hdx(tmpdir, mocker):
    mocker.patch('relativewealth.hdx.get_iso3_code', return_value='TLS')
    mock_resource = dict(
    ...
    mocker.patch('relativewealth.hdx.urlretrieve', return_value=(mock_zipped_file,None,None))
...
def test_get_hdx(tmpdir, mocker):
    ...
    mocker.patch('relativewealth.hdx.ZipFile', return_value=mock_context_object)
    cache_dir = str(tmpdir/'this-directory-does-not-exist')
